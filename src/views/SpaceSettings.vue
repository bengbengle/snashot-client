...
watch(
  ...
  async () => {
    ...
    try {
      // const uri = await getSpaceUri(props.spaceKey, import.meta.env.VITE_DEFAULT_NETWORK);
      // console.log('URI', uri);
      currentTextRecord.value = props.spaceKey;
    } catch (e) {
      console.log(e);
    }
  ...
  <Layout v-bind="$attrs">
    <template #content-left>
      ...
      <Block title="NFT">
        ...
        <a
          :href="`https://testnets.opensea.io/collection/${spaceKey}`"
          target="_blank"
          class="mb-2 block"
        >
          <UiButton
